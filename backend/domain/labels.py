...
from backend.domain.models import ParsedPerformance


def meaningful_performance_labels(performances: list[ParsedPerformance]) -> list[str]:
    """Create stable, human-facing comparison labels without provider placeholders."""
    ...
    for index, performance in enumerate(performances):
        ...
        other_notations = set().union(
            *(notations for other_index, notations in enumerate(notation_sets) if other_index != index)
        )
        distinctive = next(
            (
...
def _is_generic(performance: ParsedPerformance) -> bool:
    ...
    notations = {_compact(castell.notation).replace(" ", "") for castell in performance.castells}
