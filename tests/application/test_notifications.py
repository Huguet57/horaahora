...
def repositories() -> tuple[
    ...
    return (
        SQLAlchemyPushSubscriptionRepository(database),
        SQLAlchemyNotificationRepository(database),
    )
...
def test_subscription_registration_is_idempotent_and_rotates_the_token() -> None:
    subscriptions, _ = repositories()

    subscriptions.register(registration(), environment="production", topic="com.example.app")
    subscriptions.register(
        registration("cd" * 32), environment="production", topic="com.example.app"
    )

    active = subscriptions.list_active_subscriptions()
    assert len(active) == 1
